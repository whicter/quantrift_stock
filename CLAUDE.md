...
- **金额口径**：`BUDGET_USD = 750`（与正股纸面组合 `RISK_PCT=0.75%` 同量级，
  两边盈亏才可比），记 `contracts` / `cost_usd` / `pnl_usd`。单张成本超预算 2 倍
  直接跳过（9/4 起，记 `options_skipped.csv`）。此前只记百分比，「到底赚了多少钱」
  根本算不出来。拦截上线前硬买的 5 笔超配仓位（SNDK 单张 $16,280 等）已于 9/17
  标 `contaminated=1 / contaminated_reason=oversized`，复盘排除：它们贡献 +$2,740，
  掩盖了其余 39 笔 **-$4,266** 的真实结果。原件 `.preoversize-20260917.bak`。
...
复盘同时给美元口径，并自检账本质量（重复行、DTE 越界、作废样本及原因、剩余 DTE）。
DTE 越界只校验 9/3 硬下限生效后开的仓——此前 8/24–8/28 开的 6 笔（DTE 21–25）
曾被永远报成「违反到期下限」。
...
- **复盘只统计现役路由（2026-09-17 起）**：`signal_review.py` 默认排除已从
  ...
  SOXX/SMH/MRVL/AAPL 1d；剔除后现役 RSI2 1d 是 77 笔、均 **+0.14R**。同期回测
  按现役 STRATEGY_MAP 跑、自然没有这些路由——TASK.md 里「RSI2 1d 仅实盘有的
  11 条 -1.45R」就是这件事，已结。
...
**修了 bug 没隔离样本（2026-09-17 补）**：9/3 只做了备份，505 笔「一根 bar 就
砍掉」的旧仓仍留在实时账本里继续复利；而且 `stock-alert` 进程到 9/4 21:46 PT 才
重启，9/4 当天 21 笔平仓（18 笔 R=0）仍是旧代码判的。于是账面 **-16.99%** 里有
-7.86% 是假亏损，干净期（9/8 起 34 笔）实际 **-9.91%**（同期 SPY -1.55%）。
现已与期权账本同一处理：9/5 前平仓的仓位打 `contaminated=True`、`paper_equity.csv`
加 `contaminated` 列、权益按干净期从 100,000 重算为 90,091.81；`paper_portfolio.
CONTAMINATED_BEFORE` 记录边界。**读这两份文件做统计必须先剔除 contaminated。**
原件 `.precontam-20260917.bak`。

**教训**：凡是"回放到某个时点"的评估函数，「数据不够」和「条件达成」必须能
区分开。二者混同时，越是频繁调用（每小时扫描）错得越离谱，而且不报错、不崩溃，
只是安静地把所有统计量推向错误方向。修复之后，旧样本要隔离而不只是备份，
污染边界按**进程实际重启时点**划，不按提交时点。

## RSI2 复核（2026-09-17）

用户追问「都做了」时的第二项。结论：**是行情，不是衰减**；两处口径问题已修。

- 干净期（9/8 起）RSI2 20 笔均 **-0.74R**、胜率 15%，是纸面组合亏损的全部来源
  （Confluence 14 笔 +0.11R）。但刷新到 9/17 的同期回测 RSI2 9 月 22 笔 **-0.93R**、
  胜率 23%——回测自己在这段行情里同样亏，实盘不落后于回测。QQQ 9/10 与 9/15–16
  两度跌破 20 日低，RSI2 是「上升趋势里买回调」，这正是它必亏的行情。
- 90 天 RSI2 1d 的 -0.20R 全部来自 8/15 已下线的 4 条路由（见上），现役 +0.14R。
- 出场结构：追踪出场 171 笔均 **-1.05R**，时间止损 94 笔均 **+1.17R**——RSI2 的
  盈利全靠拿到时间上限，追踪止损几乎只在亏损时触发。这是参数层面的问题，改动属
  策略行为变更，未动。
- **R 的单位是 1 ATR，不是止损距离**（`review_core._r`）。RSI2 初始止损 1.5 ATR、
  追踪 2–3 ATR，按收盘判定还会跳空，所以单笔可达 -3.8R（MU 1h 9/14）。回测同一
  口径，比较不受影响；但 `RISK_PCT=0.75%/R` 的仓位意味着单笔真实风险 ≈ 1.1–2.3%
  权益，看「-1R」时要记得这点。
- **RSI2_IBS_shadow 未判死**：实盘 IBS<0.2 子集 -0.41R vs 其余 -0.17R，
  Mann-Whitney p=0.23，不显著；同期回测里该子集反而更好（1d +1.53 vs +0.55）。
  影子只记录不推送、零成本，继续积累。此前「可以判死」的说法撤回。
- **quality 评分无预测力，不是反向**：90 天 695 笔 Spearman rho -0.03（p=0.50），
  Confluence +0.05 / RSI2 -0.05 均不显著，与 alert_engine 里记录的 15,969 笔回放
  +0.002 一致。30 天窗口里「8-10 分桶最差」是切分噪声。它没有任何实盘消费者
  （告警不展示、meta_label 模型从未训练），只是记录字段；`_quality_report` 现在
  同时打印秩相关与 p 值，避免每换一个窗口就得出一个新方向。此前「参与推送
  优先级」的说法有误。

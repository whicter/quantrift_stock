...
> **⚠️ 2026-09-03 之前的 136 笔全部作废**（账本 `contaminated=1`，复盘自动排除）。
> `review_core.evaluate` 当时走到行情末尾返回「时间止损」，每个仓位开仓后一两根
> bar 就被判出场（中位持仓 6 小时、75% 不到 8 小时）。基于它的一切结论——赢输比、
> 价差成本、持仓时长、「RSI2 用期权显著为负」——**均已撤回**。干净样本从 9/03 起
> 重新累积。原件 `logs/options_paper_log.contaminated-20260903.bak`。
...
- **报价源 = yfinance 实时期权链**，不是 options-lab 数据库（后者 bid/ask 仅 7.6% 非空、
  ...
  该稀疏性曾把 SMH/CSCO 误判为「期权太少」，实测 OI 分别 4650/2870。
...
- **DTE = clamp(持仓上限交易日 × 3.5, 30, 60)**，优先月度到期（第三个周五）。
  ...
  **30 是硬下限**（9/03 起）：理由是流动性不是 theta——近月 vs 次月 ATM 实测
  PLTR OI 618→4,004、HOOD 208→3,161，价差双双腰斩。旧实现允许月度偏离目标 21 天，
  目标 30 时一张 16DTE 合约就被接受，全账本 49% 入场落在 30 天以内。
...
- **并发锁** `data/.options_paper.lock`：`main()` 只在结尾 `_save_state()`，两个实例
  同时跑会各平同一批仓、各写一行。首月 149 行里 23 行就是这么来的。
- **长持仓策略与期权天然不搭**：持仓越久越需要远月，而远月越没流动性。
  AVDV/VYM/FDVV/DGRO 等红利宽基 ETF 各到期都几乎无市场，只做正股。

## 期权账本复盘与白名单重测（2026-09-03 新增）

- **`options_review.py`**：核心指标是**捕获率**（正股每 1R 对应期权涨跌多少 %），
  把正股表现除掉只剩载体效率；配自助法 90% 区间一起看——点估计会骗人（污染样本
  时代 Confluence 点估计 1.20、区间 [0.23, 3.19]）。含美元口径与账本质量自检。
  已挂 `run_weekly_review.sh`，周日随正股复盘一起推 TG。
- **`options_whitelist.py`** 两条硬约束：① **非交易时段直接拒绝运行**——盘后做市商
  撤单放宽，同一合约价差是盘中 3 倍（MU 2.3%→7.9%），据此筛会把 CSCO/LLY/JPM/
  QCOM/IBM/WMT 全部误杀；② **选合约直接调 `pick_contract`**——按「30-60DTE 第一个
  到期」测可能取到周度（MU 周度 OI 34 vs 月度 3,777），必须测实盘真会买的那张。

## ⚠️ 信号与复盘口径（2026-09-03 三项修正，全部影响历史数字）

**1. review_core「数据走完 ≠ 到持仓上限」——本项目影响最大的 bug**

`evaluate` 三处 fallthrough 一律返回「时间止损」，分不清"走满 max_bars"（仓位已
了结）和"行情数据到头了"（仓位还开着）。`paper_portfolio.update()` 因此把每个刚
开的仓在下一轮扫描就判为平仓：484 笔里 459 笔「时间止损」、平均持仓 **1.87 根
bar**、**68% 只持有 1 根**（无任何路由上限是 1）。权益曲线量的是"每笔一根 bar
就砍掉"，把真实的 **+5.70%** 记成了 **-8.16%**（同期 SPY +4.19%）。
修复：`if len(future) < max_bars: return 未决`，未决保持各路径原有返回结构。
旧账本存 `logs/paper_equity.prebug-20260903.bak`。

**2. 想法级去重**——引擎无持仓状态，条件成立就每根 bar 重播同一想法。90 天 470
条信号只有 210 个独立想法（NVDA 连播 5 天、QQQ 12 条）。重播仍写 signal_log
（`is_repeat=1`）但不推送不计入复盘。推送 **-42%**，Confluence 均 R 从 +0.224 回到
**+0.289**（回测期望 +0.285）——所谓「Confluence 衰减」完全是口径假象。
判定用 `_is_repeat_idea` + `data/.open_ideas.json`，与回测「一次交易」定义对齐。

**3. 衰减基准改用同期回测**——`expectations.json` 是十年均值，拿它判定最近 90 天
等于把「这季行情不好」误读成「策略衰减」。同期回测 Confluence **-0.163**、

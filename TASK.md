...
#### ① Confluence 实盘全方向亏损归因（最高优先，先归因再调参）

- [ ] **不是"做空拖累"**：做多 −1.20R 比做空 −0.89R 更差，1h/4h/1d 全周期为负，不能简单归咎方向或 regime 过滤。
- [ ] **三个候选解释需拆开验证**：(a) 2026-06 下旬至今是震荡回撤期，趋势策略双向挨打是已知弱点，回测多年样本里本就存在这种月份；(b) 实盘信号在 yfinance bar 触发、回测在 IB bar 验证，采样锚点错位（见 N 节未纳入项）的实际影响从未量化；(c) 震荡过滤（CI/isChoppy）不够严。
- [ ] **验证方法（材料现成）**：`logs/backfill_signal_log.csv` 有 23,181 条历史已决回放，按月聚合 Confluence 均R 分布，定位当前月份 −1.0R 处于历史分布的哪个分位。**若历史上 15% 的月份同样惨 → 属正常回撤，不应改动任何参数；若超出历史范围 → 才有理由怀疑实施偏差**。
- [ ] **纪律**：归因完成前不得调整任何 Confluence 参数。

#### ② 衰减监控基线：0R 占位 → 回测期望表

- [ ] 与 ① 同源材料：从 backfill 的 23k 条已决按 `(策略, 标的, 周期)` 聚合均R/σ，生成期望表并固化。
- [ ] 红黄绿判定从"最近是否亏钱"升级为"是否显著差于该组合自身历史预期"，消除当前 0R 基线在震荡期把所有组合刷红的失真（该占位符在 N 节及周报文案中一直标注为临时方案）。
- [ ] 样本门槛沿用 N 节的 `MIN_MONITOR_SAMPLES=5`。

#### ③ Quality 星标预测力校准（疑似无效甚至反向）

- [ ] 修复后校准数据反直觉：0-4 分 −0.56R / 5-7 分 **−0.91R** / 8-10 分 +0.01R——中等分反而最差，评分公式可能无预测力，⭐ 当前或在误导人工执行。
- [ ] 先确认 `backfill_signal_log.csv` 是否记录 quality 字段；有则用大样本重新校准 `signal_pts/adx_pts/regime_pts` 权重，无则先把星标在消息中降权或移除，避免传递错误置信度。

#### ④ 新接入 90+ 标的的验证欠债（过拟合风险最高项）

- [ ] 原主池均通过 0-30bps 成本压力测试 + Walk-forward 分段验证（见 LEARNING.md「上线验证结论」），watchlist 四个批次接入的标的**全部未做**。
- [ ] 尤其 M 节网格优化救回的 36 个标的属单样本内寻优，**是全系统当前过拟合风险最高的一批**。
- [ ] 批量自动化跑成本压力 + Walk-forward，不达标者降回观察组（不直接删除，保留记录到 `watchlist_history.csv`）。

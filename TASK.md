...
- [ ] ~~修复后校准数据反直觉：0-4 分 −0.56R / 5-7 分 −0.91R / 8-10 分 +0.01R~~ **该分桶结论同样被 ① 推翻**：74/86 条实盘记录缺参数快照，其 R 值不可信，故实盘 quality 校准无效。
- [ ] 改用 `backfill_signal_log.csv`（已确认含 `quality` 字段，20,915 条已决且参数完整）做大样本校准，检验 `signal_pts/adx_pts/regime_pts` 是否真有预测力；无预测力则在消息中降权或移除星标，避免传递错误置信度。

...
#### ㉒ 盈利化改造第一批：验证 + 闭环基础设施（2026-08-15）

**背景**：用户问"整个系统还有什么需要提高的？怎么让我能够真正实现盈利"。诊断结论：
当前是**信号生成器不是盈利系统**——只有"信号是否有正期望"有数据，"你是否执行"
和"执行成本"完全是盲区。用户在方案上提出两个关键质疑，都被证明是对的：
① 影子策略优势可能是小样本假象 ② 按实盘红灯自动升降级会踩市场风格切换的噪音。

**A. Confluence 多空腿分离验证**（`confluence_direction_test.py`，47个已路由组合）
实盘 90 天做多 +0.44R(N=90) vs 做空 -0.44R(N=119)，但**回测结论是"分组合而定"，
一刀切关做空会毁掉价值**：
  - `keep_short` 16 个：做空腿确有贡献，关掉反而变差（TTD 1h 0.65→-0.41，
    DJT 1h 1.88→0.62，AMC 1h 1.08→0.57，USAR 1h 1.16→0.64）
  - `long_only_better` 12 个，其中 7 个关空后仍过 0.6 成本关（值得改）：
    BB 4h / DELL 1h / INTC 1h / MRVL 1h / MU 1h(0.27→1.46) / STX 1h / STX 4h
  - 另 5 个关空后仍不达标（LLY 4h/MU 4h/SLS 1h/STX 1d/USO 1h）——问题不在方向
  - **另一个发现**：6-7 月的差数字含已知 bug 产物（见 ⑦⑨），8 月修复后
    Confluence 做多 73 条 +0.82R

**B. RSI2 + IBS 过滤器对比**（`rsi2_ibs_test.py`，80个组合）**结论：不转正**
影子 RSI2_IBS(N=19 均R+1.83) 看似优于正式 RSI2(N=66 均R+1.38)，但历史数据不支持：
baseline 10bps Sharpe 中位数 0.265 vs +IBS 0.082，只有 44% 组合改善，胜率中位数
只 +0.1pp，信号量却从 7751 砍到 5239。**小样本假象，用户的质疑是对的。**
（`rsi2_backtest.py` 新增 `use_ibs_filter`/`ibs_threshold`，默认关闭不改现有行为）

**C. 红灯处置：从"只看"改为"看了就查"**（`decay_action.py`）
用户指出直接按实盘红灯升降级有风险。改为：连续N周红灯 → **自动用最新数据重跑
成本压力+walk-forward** → 只有重验证也挂才降级；升级永远只走完整验证流程。
首次运行即证明这个设计是必要的——9 项连续红灯中 **3 项重验证通过**
（TMC 1d 0.72 / CRWD 1h 0.695 / MU 1d 0.829，wf 全 pass），若按原方案直接
降级就是误杀。降级写入 `logs/demoted_routes.json`（数据）而非改 `STRATEGY_MAP`
（代码）——无人值守任务改源码不安全不可审计；命中的组合继续记录为影子以便
观察恢复，只是不再推送。
  - 6 项重验证也未通过：RSI2 MRVL 1d(0.203)、RSI2 AAPL 1d(0.204)、
    Confluence MU 4h(-0.09)、RSI2 SMH 1d(0.57)、Confluence NVDA 4h(-0.01)、
    RSI2 SOXX 1d(0.535)　※MRVL 1d 实为**连续8个批次**红灯，非2周

**D. 执行账本**（`execution_ledger.py` + pm2 `stock-exec-ledger`）
Telegram 长轮询接收「接 NVDA 176.5」「平 NVDA 182.3」「跳过 TSLA」「账本」，
写 `logs/execution_log.csv`，自动匹配最近信号并计算滑点与真实收益。
**这是从信号系统走向盈利系统的关键缺口**——没有它，"是否盈利"永远只能推测。
已核对 bot token 与期货项目 `ib-bot-tg-control` 不同（8864211814 vs 8882740685），
同 token 全局只能有一个 getUpdates 消费者，不冲突。绝不下单、不连 IB。

**E. 信号按板块合并推送**（用户要求"全部发出来，同板块的放在一条里发"）
不做任何过滤，只改投递方式：本轮信号先入队，扫描结束按板块合并，每板块一条消息，
同板块多条时附「⚠️ 同板块集中：做多N/做空M」——分开发时这种相关性集中度看不见。
新增 `sector_map.py`（yfinance 板块 + ETF 手工覆盖 + 本地缓存，106 个已路由标的已预热）。

**F. 顺带修复 `review_history.csv` schema 漂移**：表头停留在旧 8 字段而后续 544 行
是 11 字段（同 screener_results.csv 事故），`pd.read_csv` 直接失败导致整个文件对
下游不可用。已修写入方（追加前校验列结构）并拆分归档旧行。

- [ ] **待用户确认的策略变更**（按项目规则，参数改动需先确认）：①7个组合关做空
  ②6个重验证未通过组合降级 ③5个关空后仍不达标组合的去留

#### ㉓ RSI2-Trend 变体：为"长期趋势型"标的开发（2026-08-15，LLY 触发）

**起因**：LLY 四策略×三周期 12 个组合全不达标被降级后，用户指出"这都是好标的"，
要求为它开发合适的策略，并预判"可能其他标的也能用"——后一句正是防过拟合的关键。

**诊断**（先找病因再开药，不盲目造策略）：LLY 画像为 30% 年化波动、74% 时间在
200日线上方、日收益自相关近零——典型**低波动持续趋势型**。RSI2 卡在两处：
  ① **RS vs QQQ 过滤**：要求 LLY 跑赢纳指，但药企与科技指数比强弱本就无意义
  ② **持仓仅10根 bar**：持续趋势还没展开就被时间止损踢出
关掉 RS + 持仓放长至 30 根：LLY 1d 从 **0.157 → 0.733**，30bps 仍 0.600，
walk-forward 训练 0.722→测试 0.615。

**防过拟合（本节最重要的部分）**：该配置是从 24 个变体中挑出的最优——**与
LLY/SLS/USO 当年被网格"救回"后失效是完全相同的危险动作**。故采用事前注册假设：
  1. **先定义**（在跑验证之前）："长期趋势型" = 200SMA上方≥65% 且 年化波动≤40%，
     全池 223 个可评估标的中符合的有 67 个（占 30%）
  2. **再把配置原封不动套到全部 223 个标的**，不做任何逐标的调参
  3. **结果**：达标的 10 个里 **8 个是事前预测的趋势型（80%），而趋势型在全样本
     仅占 30%——2.7倍富集**，确认捕捉的是真实标的特征而非随机噪音

**接入 8 条 1d 路由**（零新代码：复用 RSI2 引擎，仅加 `RSI2_PARAMS` 覆盖）：
  | 标的 | 基线 | 新配置 | 30bps | DD | 备注 |
  |---|---|---|---|---|---|
  | LLY | 0.157 | 0.733 | 0.600 | -23% | 本次专项目标 |
  | CSCO | 0.292 | 0.690 | 0.542 | -26% | **此前四策略全不达标，现有解** |
  | ISRG | 0.422 | 0.699 | 0.545 | -20% | 新入扫描池 |
  | AVDV | 0.550 | 0.761 | 0.491 | -15% | 新入扫描池 |
  | VYM | 0.851 | 0.715 | 0.344 | **-11%** | 新入扫描池，回撤最小 |
  | DGRO | 0.462 | 0.700 | 0.342 | -15% | 与既有 breakout 1d 并存 |
  | TSM | 0.362 | 0.610 | 0.483 | -26% | 原仅 mr 1h |
  | CRWD | 0.422 | **0.844** | 0.773 | -32% | 不属趋势型，见下 |

  - CSCO/ISRG/AVDV/VYM 此前不在扫描池，已加入 `config.yaml watchlist_2026_07`
  - **CRWD 的判断分歧（如实记录）**：它不属于事前注册的趋势型，在 223 个标的中
    "碰巧"达标无法排除，我原建议先进观察名单；但其数值本身最强且通过项目统一
    门槛，**经用户明确决定直接接入实盘**。后续交衰减监控裁决。
- [x] **实盘验证**：重启后扫描 130/130 拉取，LLY 1d 与 CSCO 1d 做多信号即时触发并
  推送入账；`hold_description` 正确解读为"最长 30 根 1d bar（约30交易日）"

#### ㉔ 期权纸面模拟（2026-08-15，用户提议）

**用户设想**："每次给信号的时候马上模拟下单期权，到信号的止盈位置就出掉，看盈利"。

**先撤回我自己的一个错误结论**：我曾断言"RSI2 抄底恐慌 → IV 最高 → 买期权吃亏"。
用户追问"有证据吗"，三个独立检验**全部推翻**：
  | 检验 | 结果 |
  |---|---|
  | 历史 VIX 百分位（15标的~800次入场） | +4.4pp，4/15为负，弱且不一致 |
  | 实盘 signal_log 记录 VIX（143条 RSI2） | **16.24 vs 基准 16.96，反而更低** |
  | 期权库个股 ATM IV（43条，1周样本小） | **39.0% vs 42.3%，低3.3个IV点** |
  错因：套用了通用直觉，未核对我们的实际入场条件。RSI2 要求 `close>200SMA` 且
  `market_score≥2`，**这两个门槛系统性排除了恐慌行情**——它买的是平静市场里
  健康上升趋势的浅回调，不是崩盘。这反而是利好。

**可行性排查**（数据源：`quantrift_options-lab` 的 Railway PG，只读）：
  - 覆盖：105/108 已路由标的（97%），字段 IV 97.7%/delta 97.4%/OI 100%
  - **但 bid/ask 仅 7.6% 非空**，且最长只采集到 **76 DTE**
  - 故报价改用 **yfinance 实时期权链**（实测 NVDA 价差1.4%、AAPL 3.7%），
    数据库留作历史分析

**流动性筛选**（`options_liquidity.py`，按每条路由自身持仓上限匹配 DTE 而非一刀切）：
  75/108 标的可用（OI≥500），其中实测价差 <5% 的 18 个进白名单。
  **讽刺发现**：刚接入的 RSI2-Trend 一批（LLY/CSCO/ISRG/AVDV/VYM/DGRO）全部
  "无到期覆盖"——持仓上限30天需要73-147DTE，超出采集范围，且这类长趋势标的
  （红利宽基ETF）期权盘本就薄。**长持仓策略与期权天然不搭**。

**`options_paper.py` 实现**（绝不下单）。开发中实测暴露并修复四个真实缺陷：
  1. **影子信号被当实盘开仓**：`is_shadow` 是浮点 `1.0`，用 `astype(str)!="1"`
     比较失效 → 改数值比较 + 策略名后缀双保险
  2. **同一想法重复开仓**：连续 bar 反复触发被开成多仓，会把单次判断的盈亏
     重复计数 → 按 (标的,周期,策略,方向) 去重
  3. **周度到期流动性差**：实测 CRWD 同时点周度价差16.0%/OI99 vs 月度8.4%/OI233
     → 优先取月度到期（第三个周五）
  4. **时点错配（最严重）**：初版加了个 3 天回溯窗口去捞历史信号，导致用今天的
     期权报价去对应几天前发出的信号——实测 MU 正股+1.36R vs 期权-1.7%，两边
     根本不是同一段时间。用户直接点破："当然是出信号的时候下期权"。
     → 改为只接 `FRESH_MINUTES=30` 内发出的信号，按当下报价开仓。本任务在主扫描
     （:00，数分钟跑完）后的 :10 触发，窗口刚好覆盖当轮、不会捞到上一小时的。
     曾另加"是否仍未出场"的二次判定，但它会因 bar 粒度（1h 信号 bar_date 当天
     含7根bar）把刚发出的信号误判成已出场，属误杀，已移除——新鲜度窗口本身
     就是充分且正确的闸门。
  **成交口径（2026-08-15 按用户要求调整）**：初版设了 8% 实时价差闸门并按
  ask/bid 成交，用户明确指示"价差大没关系，就下中间价，不要过滤"。故：
  - 闸门取消，白名单准入条件从"价差<5%"改为"**ATM 未平仓量≥500**"
    （有没有真实双边市场才是硬条件，价差宽窄由使用者承担）→ 覆盖从 18 扩到 **75 个标的**
  - 主口径改为 **mid→mid**；**保守口径（买ask/卖bid）仍作为对照列保留**，
    两者之差即价差成本，也是"限价单能否在 mid 成交"这一假设的敞口。
    宽价差标的上两个数会差很多，账本两个都记，分析时可自行取舍。
**白名单与 DTE 的两处修正（2026-08-15，用户追问"SMH为什么期权少"引出）**：
  - **SMH/CSCO 被误排除**：筛选脚本读 options-lab 数据库，而该库恰好没收这两个
    （连同 FOF 共3个未覆盖），被归入"无到期覆盖"。yfinance 实测两者流动性其实
    很好（SMH 34DTE 价差7.2%/OI4650/量489，CSCO 34DTE 价差4.9%/OI2870/量2012），
    优于名单内不少标的。**这是数据采集缺口被我当成了市场事实**，已手工补入。
  - **DTE 必须封顶**：原规则机械地取持仓×3.5，把长持仓路由推到无人交易的远月——
    LLY 持仓30天→105DTE 处 OI 仅 39，而它 34DTE 处 OI 有 602。远月省下的 theta
    远不抵流动性损失。改为 `min(max(30, 持仓×3.5), 60)`。
  - 封顶后复核此前被排除的一批，准入线定为**目标到期处 OI≥50 且有有效双边报价**：
    补入 LLY(77)/ISRG(64)/NTRS(61)/MAR(217)/SPYM(291)/VOO(95)/VTI(550)；
    仍排除 AVDV(OI0)/VYM(6)/FDVV(11)/MKSI(32)/SPMO(38)/DGRO(46) 这些几乎无市场的。
  - 白名单最终 **84 个标的**。注意 ISRG/NTRS 等价差 20-37%，mid 成交假设偏乐观，
    账本同时记保守口径供对照。
  - 顺带澄清 **SPXC ≠ SPCX**：SPXC=SPX Technologies（工业，1h RSI2 已路由，
    网格 Sharpe 1.058）；SPCX=Space Exploration Technologies（2026-06-12上市，
    用户要求接入但未经回测，账本标 `promoted_unvalidated`）。两个都真实存在。

- [x] pm2 `stock-options-paper` 每小时 :10 运行（主扫描之后）
- [x] **2026-08-19 补：接入 Telegram 推送**。用户反馈"模拟期权单我好像并没有看到"
  ——查证属实：`options_paper.py` 完全没接 Telegram，pm2 配置里连 TG_TOKEN 都没传，
  它三天来一直静默跑只写 CSV。已补：开仓/平仓各合并成一条消息推送（含合约、
  mid/bid/ask、价差、IV、OI，平仓时对比正股R与期权两种口径）。
  env 变更需 `pm2 delete` + `start`（`restart` 不重载 env），已重建并 save。
- [x] **首批 45 笔结果（08-17~08-19）**：期权 mid口径胜率11%/均-6.6%，保守口径
  胜率4%/均-14.2%，**同期正股也在亏**（胜率24%/均-0.45R），故不能据此判定期权好坏。
  **真正有信息量的是不对称性**：正股盈利的11笔均 +0.35R，期权只有 **+1.2%**；
  正股亏损的34笔均 -0.71R，期权 **-9.1%**。亏损被杠杆完整放大，盈利却被
  theta+价差吃掉大半。价差成本实测平均 **7.6 个百分点**（两口径之差）。
  38/45 笔持仓不足1天，说明这不是长期 theta 衰减，主要是价差。
- [ ] **样本仍不足**：3 天 45 笔虽快，但正股同期表现（-0.45R）显著差于90天基准
  （+0.35R），属不利区间。需跨越不同市场环境后再评估。
- [ ] **已知限制**：盘后 yfinance 期权 bid/ask 归零，模块只在盘中有效，
  盘后整点运行会静默跳过（代码拒绝在无双边报价时编造价格，属正确行为）。

- [ ] **待积累样本**：需等新信号自然触发，账本 `logs/options_paper_log.csv` 攒够
  30+ 笔后才能回答"期权是否比正股更划算"
...
### ㉖ 模拟栈口径修复与期权模拟重建（2026-09-03）

用户追问「你复盘盈亏了吗」→ 顺藤摸出整条模拟链在量错东西。本轮共修 12 处。

**最严重：`review_core` 数据走完 ≠ 到持仓上限**
`evaluate` 三处 fallthrough 一律返回「时间止损」。`paper_portfolio` 因此把每个刚
开的仓在下一轮扫描就判为平仓——484 笔里 459 笔「时间止损」、平均持仓 1.87 根 bar、
68% 只持有 1 根。**权益从真实的 +5.70% 被记成 -8.16%**（同期 SPY +4.19%）。
`options_paper` 同样中招 → **136 笔期权记录全部作废**，基于它的一切结论已撤回。

**信号侧**
| 缺陷 | 影响 |
|---|---|
| 引擎无持仓状态、重播信号 | 推送 -42%；Confluence 均 R +0.224→+0.289（期望 +0.285） |
| 衰减监控用十年均值当基准 | 批量误报红灯 → 错误降级；改同期回测后红灯收敛到 11 项 |
| `_YF_PERIOD["4h"]="60d"`（只得 ~119 根 bar，SMA200 需 210）→ 改 `730d`（~1,707 根） | 36 条 RSI2 4h 路由**从上线起零信号** |
| 实盘漏「QQQ 破20日低」罚分 | 大盘创新低时该收紧却放行 |
| `historical_backfill` 崩溃 6 周 | 对照失效，上面那 36 条死路由因此没被发现 |
| IB 夜间保鲜静默失败 13 天 | 24 个标的日线、25 个 1h/4h 停在 8/21 |

**期权模拟重建**
- 出场判定改用与引擎同源的行情（`load_price` → `fetch_bars`）
- 金额口径：`BUDGET_USD=750` + `contracts`/`cost_usd`/`pnl_usd`
- 归因字段：两端 IV 与标的价
- `options_review.py` 挂周日复盘；`options_whitelist.py` 周三盘中重测
- 136 笔污染样本标记 `contaminated=1` 并从复盘排除

**教训（已写入 LEARNING.md）**
- 凡"回放到某时点"的评估函数，「数据不够」与「条件达成」必须能区分
- 任何"实盘 vs 预期"的比较，基准必须来自同一段行情
- 测流动性只能盘中测，且必须复用实盘的选合约逻辑
- 当筛选结果要剔除掉所有最显而易见的优质样本时，先怀疑测量本身

**待办**：白名单等周三盘中数据；~~RSI2 1d 仅实盘有的 11 条信号均 -1.45R 待查~~
（9/17 已结：是 8/15 下线的 4 条 1d 路由，复盘现已排除已下线路由，见 ㉗）；
干净期权样本累积中（当前 5 笔）。**用户至今零条真实成交记录**——手动 TG 记账的
设计三周无人使用，说明设计本身不成立。

### ㉗ 模拟账本隔离与 RSI2 复核（2026-09-17）

用户「复盘一下最近的所有模拟交易」→ 三项按序处理完毕。

**1. 账本隔离**
- 正股纸面：9/3 修 bug 后旧样本只备份未隔离，且进程 9/4 21:46 才重启。505 笔
  9/5 前平仓的仓位打 `contaminated=True`，`paper_equity.csv` 加 `contaminated`
  列，权益按干净期从 100,000 重算 → **90,091.81**（干净期 34 笔 -9.91%，同期
  SPY -1.55%）。账面 -16.99% 作废。原件 `.precontam-20260917.bak`。
- 期权账本：9/4 拦截上线前硬买的 5 笔超配仓位标 `contaminated=1 /
  contaminated_reason=oversized`（新列）。剔除后 46 笔 -$4,266（此前 -$1,526
  被 SNDK 一笔 +$3,870 掩盖）。DTE 越界校验只看 9/3 下限生效后开的仓。

**2. RSI2 复核**
- 90 天 RSI2 1d -21.5R 全部来自 8/15 已下线的 SOXX/SMH/MRVL/AAPL 1d（29 笔
  -32.4R）；现役 1d +0.14R。`signal_review.py` 默认排除已下线路由
  （`--include-retired`）。
- 9 月：干净期实盘 -0.74R vs 刷新到 9/17 的同期回测 -0.93R → **行情，不是衰减**
  （QQQ 两度破 20 日低）。未动任何路由或参数。
- 出场结构：追踪出场 171 笔 -1.05R / 时间止损 94 笔 +1.17R——策略参数层问题，
  属行为变更，留待用户决定。
- RSI2_IBS_shadow：实盘子集差但 p=0.23，回测子集反而好 → 不判死，继续记录。

**3. quality 评分**
- 90 天 695 笔 Spearman rho -0.03（p=0.50）：无预测力，非反向；无实盘消费者。
  `_quality_report` 现打印秩相关与 p 值。

**未做（需用户决定）**：RSI2 追踪止损参数是否放宽；当前 23 笔在场里 15 笔 RSI2
且集中 1d/4h。

### ㉘ IB 夜间保鲜每轮腰斩：真因是 Gateway 14:30 自重启（2026-09-18）

TG 告警「56/124 个标的未更新」。两个叠加的问题：

1. **重连代码从上线起就是坏的**：9/11 加的断线重连引用了未定义的 `IB_HOST`，
   每次 `NameError`、三次后中止本轮。9/15–9/18 每轮剩下 56–57 个标的全部报废。
   已补常量；重连窗口从 3×15s 放宽到 6×30s（够覆盖一次 Gateway 重启）。
2. **断线本身不是限流**：IBC `AutoRestartTime=02:30 PM`，Gateway 每天 14:30 PT
   自重启；任务 14:00 开跑、一轮约 55 分钟，必然被切断。证据：9/17、9/18 最后
   写入都在 14:30；9/18 Gateway 进程启动于 14:30:04；14:32 手动补跑连续 30 分钟
   133 个请求零断线。9/11 归因到限流并把间隔从 6s 改到 12s 是**错的**（12s 保留，
   因为 6s 确实超出 60 请求/10 分钟的限额）。
   调度 `0 14` → **`40 14`**，14:40 开跑、约 15:35 结束。

9/18 14:32 手动补跑覆盖了受影响的标的（最陈旧的排最前）。

### ㉙ 符号链接被写入方替换，外置盘迁移每周白做（2026-09-23）

用户问「有什么数据可以存到外接硬盘」。本项目已无多少可搬（2,799 个 CSV 早已是
符号链接，外置盘上 6.5G），但 `data/` 还有 370 个实体文件 104M，且 consolidate
日志显示每周都在搬同样数量——搬走的又回来了。

根因：`save_bars` 的 `os.replace(temp, path)` 在 `path` 是符号链接时替换链接本身
（rename 不跟随符号链接）。证据：370 个实体文件 mtime 全部落在上一次夜间刷新
（9/22 14:43–15:11）的时间窗内。

修复：`consolidate_data.write_target()` 统一 resolve，`fetch_ib_data`/`fetch_data`
的 `save_bars` 都改为写穿到外置盘真实路径，临时文件放目标同目录。
验证：单元级（临时目录造链接写入，链接保留、目标更新）+ 端到端（`--symbol NVDA
--tf 1d --merge` 真实拉取，链接保留、外置盘文件时间戳更新、无残留 .tmp）。
`data/` 104M → 2.8M。

**未做**：`ai_snowboarding/data`(5.0G)、`quantrift_index_future` 的 data/research
(1.8G) 迁移涉及其他仓库，待用户确认；本项目 `logs/backfill_*.csv`(30M) 可搬但会
给周复盘的同期基准加一个外置盘依赖，收益太小，暂不动。

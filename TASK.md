...
- [x] **MR（均值回归）策略补测未达标标的**：对本轮所有未达标的 126 个候选标的（排除已知货币基金/杠杆ETF 19个后共107个）跑 `mr_backtest.py` 默认参数回测，覆盖 1h/4h/1d。结果：**仅 2 个达标**——`TSM` 1h Sharpe 1.281 N=31（勉强过 N≥30 门槛）、`FDVV` 1h Sharpe 0.619 N=38；其余 105 个（含用户关心的 `COIN` 1h Sharpe -0.618、`EU` 1h Sharpe -0.621）在 MR 框架下依然不达标，甚至比 Confluence/RSI2 更差。**发现架构缺口**：`mr_signals.py`/`mr_backtest.py` 从未接入 `alert_engine.py` 实时扫描——`STRATEGY_MAP` 分发逻辑只识别 `confluence`/`rsi2`/`breakout`，没有 `check_mr_signal()` 函数。鉴于只有 2 个样本量刚过线的标的达标，新建一套实时 MR 检测通道（含 ATR 追踪止损状态机、Telegram 消息格式）投入产出比低，**本轮不实现，仅记录结论**，TSM/FDVV 未接入 alert_engine。

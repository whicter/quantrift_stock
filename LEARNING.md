...
## ⚠️ 撤回声明：下面这节「期权纸面模拟首次复盘」的数据结论全部作废（2026-09-03 当日）

写完那节之后才发现 `review_core.evaluate` 走到行情数据末尾会返回「时间止损」，
于是每个期权仓位在开仓后一两根 bar 就被判为出场。**那 126 笔样本的持仓时长本身
就是 bug 产物**，因此建立在它之上的一切数字都不成立：

| 当时的结论 | 状态 |
|---|---|
| 「实际持仓中位 6 小时、最长 4.2 天」 | ❌ 是提前平仓造成的，不是策略特征 |
| 「价差成本 9.2 个点/笔」 | ❌ 作废 |
| 「赢输比 0.73，亏损被放大得更多」 | ❌ 作废 |
| 「RSI2 用期权显著为负 [0.20, 0.72]」 | ❌ 作废 |
| 「出场时最少还剩 15.7 天 DTE，theta 没咬到」 | ❌ 前提不成立 |

**唯一仍然成立的是方法论部分**（教训一「开了单不复盘等于没做」、教训二「并发写入
必须加锁」、教训三「换参数前先验证给出的理由本身成立」、教训四「捕获率比孤立的
期权收益率更有信息量」这个**指标设计思路**本身）。数字一律以 9/03 之后重新累积的
干净样本为准。

保留原文不删，是因为「拿被污染的样本一本正经算出置信区间」这件事本身值得留作
警示——自助法区间算得再规范，样本是错的就全错。
...
## 衰减监控的基准选错了（2026-09-03）

`expectations.json` 里的 `mean_r` 是**十年样本的长期均值**（Confluence +0.285、
RSI2 +0.388）。衰减监控拿它当基准判定最近 90 天的实盘，等于把"这一季行情本来
就不好"误读成"策略衰减"。

同期回测（`backfill_paper_equity.csv`，只算真正开过仓的行）最近 90 天：

| 策略 | 长期均值 | 同期回测 | 实盘（去重后） |
|---|---|---|---|
| Confluence | +0.285 | **-0.163** | **+0.289** |
| RSI2 | +0.388 | **-0.060** | -0.110 |

也就是说 Confluence 实盘其实**跑赢**了同期回测，而我一度报告它"落后 0.44R"。
用长期均值当基准，监控会在每一段低于平均的行情里批量误报红灯，进而触发自动
降级——**正好把策略在最不该关的时候关掉**，这正是用户此前担心的场景。

改成同期回测基准后，红灯从"遍地"收敛到 11 项，且集中在 **RSI2 1d**（CSCO/
MRVL/LLY/AAPL/SMH/SOXX/QQQ 七项）：同期回测 +0.57，实盘却普遍为负。这才是
真正需要查的一条线索。

**方法论**：任何"实盘 vs 预期"的比较，基准必须来自**同一段行情**。跨期比较
只能回答"这段行情好不好"，回答不了"策略有没有坏"。另外统计同期基准时要排除
`skip_*` 的行——它们代表"没开成的仓"，混进来会稀释基准。

## 盘后测期权价差 = 白测（2026-09-03）

用实时链重建期权白名单，跑出来的结论是要把 CSCO / LLY / JPM / QCOM / IBM /
WMT / GE / BABA 全部剔除——全是流动性最好的大盘股。这种结论本身就是错误信号。

两个原因叠加：

1. **在纽约时间 22:51 跑的**，收盘后近 7 小时。做市商撤单/放宽，同一张合约的
   价差是盘中的 3 倍：MU 2.3%→7.9%、PLTR 2.2%→7.6%、HOOD 2.5%→7.4%。
2. **选合约逻辑没复用实盘的**。脚本取"30-60DTE 里的第一个到期"，可能是周度
   合约；实盘 `pick_contract` 是优先月度。实测 MU 周度 OI 只有 34，月度 3,777。
   测的不是我们会买的那一张，数字再准也没意义。

**方法论**：任何测量流动性/价差/盘口的脚本，(a) 必须在盘中跑，(b) 必须复用实盘
的标的选择逻辑。已把这两条写成 `options_whitelist.py` 的硬约束——非交易时段直接
拒绝运行并说明原因，选合约直接调 `options_paper.pick_contract`。

更一般的教训：**当一个筛选结果要剔除掉所有最显而易见的优质样本时，先怀疑测量
本身，而不是接受结论。** 这次是第二次栽在同一类问题上——上一次是从 bid/ask 只有
7.6% 覆盖的期权库里得出"SMH/CSCO 期权太少"，同样是测量工具的问题被当成了事实。

## 三条口径教训（2026-09-03，均为当天实测）

### 一、「数据不够」与「条件达成」必须能区分——否则越勤快错得越离谱

`review_core.evaluate` 三处 fallthrough 一律返回「时间止损」，把两件语义相反的事
混为一谈：走满 `max_bars`（仓位已了结）和走到行情数据末尾（仓位还开着）。

`paper_portfolio.update()` 每小时调它一次，于是每个刚开的仓在**下一轮扫描**就被
判为平仓：484 笔里 459 笔「时间止损」、平均持仓 **1.87 根 bar**、**68% 只持有
1 根**——而没有任何路由的持仓上限是 1。权益曲线量的根本不是策略，是「每笔交易
一根 bar 就砍掉」，把真实的 **+5.70%** 记成了 **-8.16%**。`options_paper` 同样
中招，账本里「持仓中位 6 小时」是 bug 产物而非策略特征，136 笔全部作废。

**这个 bug 不报错、不崩溃**，只是安静地把所有统计量推向错误方向；而且调用越频繁
偏得越远。凡是"回放到某个时点"的评估函数，都必须能区分这两种情况。

修复时还有一个次级教训：**返回结构不能随出场与否而变**。测试靠 `"ambiguity"` 键
判断走了哪条评估路径，未决分支少了这个键就把测试打挂了——调用方依赖的是结构。

### 二、任何「实盘 vs 预期」的比较，基准必须来自同一段行情

`expectations.json` 的 `mean_r` 是十年均值。拿它判定最近 90 天，等于把「这一季
行情本来就不好」误读成「策略衰减」。同期回测（**只算真正开过仓的行**，`skip_*`
代表没开成的仓、混进来会稀释基准）：

| 策略 | 十年均值 | 同期回测 | 实盘（去重后） |
|---|---|---|---|
| Confluence | +0.285 | **-0.163** | **+0.289** ← 跑赢同期 |
| RSI2 | +0.388 | -0.060 | -0.110 |

我一度据此向用户报告「Confluence 落后 0.44R」，是错的。更严重的是这套基准喂给了
自动降级链：长期均值会在每段低于平均的行情里批量误报红灯，**正好把策略在最不该
关的时候关掉**——用户此前担心的正是这个场景。改用同期基准后红灯从遍地收敛到 11 项。

### 三、统计口径的污染会伪装成「策略衰减」

引擎没有持仓状态，入场条件还成立就每根 bar 重播同一个想法：90 天 470 条信号只有
**210 个独立想法**（NVDA 连播 5 天、QQQ 12 条）。回测一个想法只开一次仓，实盘却
把每次重播都当独立交易——而重播是同一波行情里越来越晚的入场点。

去重后 Confluence 均 R 从 +0.224 回到 **+0.289**，回测期望 +0.285，**缺口 +0.004**。
所谓的「Confluence 衰减」完全是口径造出来的假象。

判定「是否同一想法」用回测的出场逻辑，而不是「隔几天算新的」这类阈值——这样实盘
与回测对「一次交易」的定义才真正对齐。

### 附：4h 路由死了整整一个上线周期，而对照工具也是坏的

`_YF_PERIOD["4h"]="60d"` 重采样只得 ~119 根 bar，而 `check_rsi2_signal` 开头
`if len(df) < 210: return None`（SMA200 要 200 根）——**36 条 RSI2 4h 路由从上线
起一条信号都没发过**，signal_log 里 RSI2/4h 累计为 0。改 `730d` 后有 ~1,707 根。

为什么没人发现：`historical_backfill.py`（重放"策略逻辑本应产生哪些信号"、也是唯一
能与实盘对照的工具）自 2026-07-26 起就在 `RSI2_PARAMS[(symbol, tf)]` 上 KeyError
崩溃——那天 MU/STX 1h 从 confluence 升级成 rsi2、没有专属参数。**对照工具坏了六周，
所以死路由无人察觉。** 同理，`fetch_ib_data.py` 的 Gateway 中途掉线也是静默的
（照常跑完、退出 0、pm2 认为成功），24 个标的的数据停了 13 天。

**监控工具本身也需要监控。** 静默失败的检查器，比没有检查器更危险——它给人一种
"有人在看"的错觉。

## 修了 bug 不等于清了账；关了路由不等于出了统计（2026-09-17）

用户让我「复盘一下最近的所有模拟交易」，头条数字是纸面权益 -16.99%。追下去发现
三件事，每一件都不是策略问题，而是复盘自己在骗自己。

### 一、修复之后旧样本要隔离，污染边界按进程重启时点划

9/3 修了 review_core 的提前平仓 bug，做了备份，然后**旧账本原样留在实时文件里
继续复利**。更隐蔽的是：修复提交在 9/3 19:17，但 `stock-alert` 是长驻进程，到
9/4 21:46 才重启——9/4 当天 21 笔平仓里 18 笔 R=0，和修复前一模一样。按平仓日
逐日看 `bars` 中位数（1.0 → 15.0）能一眼定位切换点，按提交时间划会漏掉一天。

账面 -16.99% 里 -7.86% 是假亏损；干净期 34 笔 -9.91%。期权账本 9/3 做对了
（`contaminated=1` + 复盘排除），正股账本没做，同一个 bug 两种处理。现在统一。

### 二、复盘对象必须是现役配置

90 天 RSI2 1d 均 -0.20R，看起来 1d 路由整体不行。拆到标的：SOXX/SMH/MRVL/AAPL
四条 1d 路由 29 笔 -32.4R，而它们 **8/15 就已按方向/衰减验证下线**。剔除后现役
RSI2 1d 是 77 笔 +0.14R。同期回测按现役 `STRATEGY_MAP` 跑，自然没有这些信号——
TASK.md 上挂了两周的「RSI2 1d 仅实盘有的 11 条 -1.45R 待查」就是这个，不是
对齐缺陷，是复盘把已经关掉的东西继续算进「衰减」。每周红灯里有多少是这种
早已处理过的问题在反复报警，没人数过。

### 三、「行情 vs 衰减」只有同期回测能裁决，而回测基准也会过期

干净期 RSI2 -0.74R、胜率 15%，是纸面组合亏损的全部来源。判定这是衰减还是行情，
唯一办法是看同期回测——但 `backfill_paper_equity.csv` 停在 9/2，无法裁决 9 月。
刷新到 9/17 后：同期回测 RSI2 9 月 22 笔 **-0.93R**，比实盘还差。QQQ 两度跌破
20 日低，「上升趋势里买回调」在这种行情里必亏。**是行情。** 若没刷新基准，我会
把它报成衰减——和 9/3 那次用十年均值当基准犯的是同一类错。

### 四、分桶差异先看秩相关，再决定要不要相信方向

30 天窗口里 quality 8-10 分桶均 R -0.60、0-4 分桶 -0.64，我据此写了「评分反向」。
90 天 695 笔的 Spearman rho 是 -0.03（p=0.50）：**没有预测力，既不正向也不反向**，
与 alert_engine 里早就记录的 15,969 笔 +0.002 一致。三个桶、几十个样本，换个
窗口就换个方向。现在 `_quality_report` 把秩相关和 p 值一起打出来。

同样的教训用在 RSI2_IBS_shadow 上：实盘子集 -0.41 vs -0.17，p=0.23；同期回测里
子集反而更好。我先说了「可以判死」，证据不够，撤回。影子零成本，继续记。

### 五、R 的单位要说清楚

`review_core._r` 把 1R 定义为 1 ATR，不是止损距离。RSI2 初始止损 1.5 ATR、追踪
2–3 ATR、按收盘判定还会跳空，单笔 -3.8R（MU 1h）不是 bug。但 `RISK_PCT=0.75%/R`
的仓位意味着单笔真实风险约 1.1–2.3% 权益。回测同口径，比较不受影响；解读绝对
数字时要记得。

## 「搬走的文件又回来了」= 写入方把符号链接替换掉了（2026-09-23）

用户问「有什么数据可以存到外接硬盘」，查下来本项目已经没什么可搬的——2,799 个历史 CSV
早就是指向外置盘的符号链接。但 `data/` 里还有 370 个实体文件、104M，而 consolidate 日志
显示它**每周都在搬同样数量的文件**（286–369 个）。不是没搬干净，是搬走的又回来了。

根因：`os.replace(temp, path)`。POSIX rename **不跟随符号链接**，`path` 是链接时被替换的
是链接本身，外置盘上的目标文件原地不动，本地多出一个实体文件。而同一个文件用普通
`to_csv(path)` 写就没事——`open()` 跟随符号链接。所以「原子写」这个更谨慎的写法，恰恰是
破坏链接的那个。

定位方法很简单：那 370 个实体文件的 mtime **全部落在上一次夜间刷新的时间窗内**。
「一批文件的 mtime 高度集中」基本就指明了是哪个任务写的。

延伸：把存储位置做成符号链接后，所有写入方都变成了这层间接的利益相关者，而它们对此一无
所知。迁移完成的判据不该是「搬完了」，而是「搬完后跑一轮真实写入，链接仍在」。

## 断在固定时刻，先查时钟，别查速率（2026-09-18）

9/11 夜间 IB 保鲜每轮中途 `Peer closed connection`，我归因到限流（6s 间隔超出
60 请求/10 分钟），调到 12s，同时加了断线重连。一周后仍然每轮断。两处错：

- **重连引用了未定义的 `IB_HOST`**，从上线起一次都没成功过。重连只在异常路径
  执行，正常路径测不到——而我当时没有造一次断线去验它。
- **归因错了**。真相是 IBC `AutoRestartTime=02:30 PM`：Gateway 每天 14:30 自重启，
  任务 14:00 开跑、55 分钟跑不完，必然腰斩。9/11 那 194 个请求 × 6s 约 29 分钟，
  同样落在 14:30——那时就有这条线索，但我看的是「请求数」，没看「钟点」。

鉴别方法很便宜：请求速率减半后断点的**钟点**没变（都在 14:30），而断前请求数
变了（194 → ~135），说明决定因素是时间不是速率。再看 Gateway 进程启动时间
（`ps -o lstart`）就是铁证。凡是「每次都断在差不多的地方」，先把断点换算成
挂钟时间，对照同机所有定时任务与守护进程的计划表。
